...
# -*- coding: utf-8 -*-
"""
Funcion OPCIONAL que utiliza la informacion de prefijos mas especificos presente en ficheros CSV de cada colector independiente, 
para realizar experimentos teniendo en cuenta unicamente los prefijos base que tienen longitud de prefijo 24.
Unicamente tiene en cuenta prefijos de tipo IPv4
...
def almacenar_diccionario_mas_especificos_final(dicc_ASOrigen_esp):
    ...
    for i,v in dicc_ASOrigen_esp.items():
		...
		# Añado la informacion unicamente si es un prefijo de longitud 24
        if len(v[0]) == 1:
...
dataframe_csv_mas_especificos_ipv4.to_csv("ResultadosCSV/Mas_Especificos/DiaRCC/MasEspecificos_ipv4_global_24_" + fecha  +  ".csv",index=False)
