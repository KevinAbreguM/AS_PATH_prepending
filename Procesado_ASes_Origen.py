...
# -*- coding: utf-8 -*-
"""
Codigo que procesa la informacion de los CSVs a nivel ASes Origen (que originan rutas) y 
...
def representacion_pie(tamano,lista,labels,title_leyenda,title,num_imagen,fecha,url_imagen,nombre):
	...
	#Creo la figura de 1 fila y 1 columna del tamaño indicado
	...
	# Se añade caracteristicas de la leyenda, texto y titulo
...
'''
def informacion_general_monitores(lista_monitores_origen,data_origen):
    ...
    for data_origen in lista_monitores_origen:
        
        #Creacion y utilización de mascaras para el monitor y para la realización de AS_PATH prepending
        mask_monitor = data_origen["Monitor"] == data_origen
        df_monitor = data_origen[mask_monitor]
        ...
        try:
            ...
            diccionario_monitores_ASes_validos.update({data_origen: len(df_monitor)})
			...
			#Almacenamiento de estadisticas de monitor
            diccionario_monitores_ASes_validos_ipv6.update({data_origen: len(df_monitor)})
...
def representacion_1_figura_diccionarios(diccionario,tamano,bar_width,opacity,
    ...
    #Añado informacion sobre la grafica
    ax.set_xticks(index)
...
def representacion_1_figura_contadores(contador,tamano,ylabel,xlabel,title,num_imagen,fecha,url):
	...
	#Añado informacion sobre la grafica y los ejes
    ax.set_ylabel(ylabel)
    ax.set_xlabel(xlabel)
    ax.set_title(title)    
    ax.grid()
...
'''informacion_ASorigen_prepending_cuantos_monitores:
...
'''
def informacion_ASorigen_prepending_cuantos_monitores(data_origen):
    
    diccionario_ASorigen_info = {}
    ...
    for monitor in data_origen.Monitor.unique():
        ...
        lista_AS_monitor = data_monitor.ASOrigen.unique().tolist()
        lista_AS_monitor_prep = data_monitor_prep.ASOrigen.unique().tolist()
        ...
        diccionario_ASorigen_info.update({monitor:[lista_AS_monitor,lista_AS_monitor_prep]})

    diccionario_ASorigen_infototal = {}
	
	#Recorro el anterior diccionario
    for i,v in enumerate(diccionario_ASorigen_info.items()):
        ...
        for AS in v[1][0]:
            ...
            if not AS in diccionario_ASorigen_infototal:            
            
                diccionario_ASorigen_infototal.update({AS:[prepending,1,1]})
            
            else:
            
                lista_auxiliar = diccionario_ASorigen_infototal.get(AS)
                diccionario_ASorigen_infototal.update({AS:[lista_auxiliar[0]+prepending,
                                                      lista_auxiliar[1]+1,lista_auxiliar[2]+1]})
	
	#Obtengo el numero de ASes distintos contando la informacion de todos los colectores
    num_ASes =  len(diccionario_ASorigen_infototal)
    ...
    for i,v in enumerate(diccionario_ASorigen_infototal.items()):
...
'''informacion_ASorigen_prepending_siempre_cuantos_monitores:
	Funcion que devuelve informacion relacionada con los ASes.
	...
	Mientras que en esta ocasion debe realizarlo en todas las apariciones del AS por monitor
		...
		Mismas ultimas 3 pruebas pero con ASes que aparecen en mas de 300 monitores distintos.
'''
def informacion_ASorigen_prepending_siempre_cuantos_monitores(data_origen):
    ...
    for monitor in data_origen.Monitor.unique():
        ...
        C = np.where((data_monitor_prep["Prepending"] == data_monitor_prep["Totales"]),data_monitor_prep["ASOrigen"],"0")
        lista_aux = C.tolist()
        lista_AS_monitor = data_monitor.ASOrigen.unique().tolist()
    ...
    for i,v in enumerate(diccionario_ASorigen_info.items()):
        ...
        for AS in v[1][0]:
			...
			#Compruebo si ese AS ha realizado AS_PATH prepending siempre
            if AS in v[1][1]:
...
url_imagen = "ResultadosCSV/ASes_imagenes/DiaRCC/AS_origen_"

start_time = time()

#Se agrupa todos los datos de los distintos CSV en un unico dataframecontador = 0
for version_rrc in rrc:
    ...
    if contador == 0:
        ...
        data_origen = data_origen.append(pd.read_csv(url_origen+fecha+version_rrc+".csv",sep=",",dtype={"ASOrigen": str}),ignore_index = True)
...
lista_monitores_origen = data_origen.Monitor.unique().tolist()
...
[diccionario_monitores_ASes_validos,diccionario_monitores_ASes_validos_ipv6,
 ...
 lista_porc_prep_nunca_ipv4,lista_porc_prep_nunca_ipv6] = informacion_general_monitores(lista_monitores_origen,data_origen)
...
num_imagen = representacion_1_figura_lista_ordenada(lista_ordenada_monitores_ASes_validos,(10,5),bar_width,opacity,
                                        "Número ASes vistos","Monitores",title,num_imagen,fecha,url_imagen,"IPv4")
...
lista_ordenada_monitores_ASes_validos_ipv6 = eliminar_valor_lista_alto(lista_ordenada_monitores_ASes_validos_ipv6,20000)
num_imagen = representacion_1_figura_lista_ordenada(lista_ordenada_monitores_ASes_validos_ipv6,(10,5),bar_width,opacity,
                                        "Número ASes vistos","Monitores",title,num_imagen,fecha,url_imagen,"IPv6")
...
num_imagen = representacion_1_figura_lista_ordenada(lista_prep_ipv4,(10,5),bar_width,opacity,
                                        "Número ASes vistos","Monitores",title,num_imagen,fecha,url_imagen,"IPv4")
title = ""
lista_prep_ipv6 = eliminar_valor_lista_alto(lista_prep_ipv6,2000)
...
#Representacion de en cuantos monitores distintos aparece el AS origen 
lista_distintos_monitores = data_origen.groupby("ASOrigen").size().tolist()
...
num_imagen = representacion_1_figura_contadores(contador_distintos_monitores_repeticiones,
            (10,5),"Número ASes distintos","Monitores distintos",title,num_imagen,fecha,url_imagen)
...
[num_ASes,num_no_prepending,num_algunos_prepending,num_todos_prep,
 ...
 num_todos_prep_filt] = informacion_ASorigen_prepending_cuantos_monitores(data_origen)
...
labels_dicc = ["Todos", "Algunos",
               "Ninguno","Número ASes totales"]

title = ""
num_imagen = representacion_1_figura_lista(lista_info_final,labels_dicc,(10,5),"Número ASes Origen distintos",
...
title = ""
num_imagen = representacion_1_figura_lista(lista_info_final_porcentaje,labels_dicc,(10,5),"Número ASes Origen distintos",
                "Monitor",title,num_imagen,fecha,url_imagen,bar_width,opacity,'vertical')
...
title = ""
num_imagen = representacion_1_figura_lista(lista_info_final_filtrado,labels_dicc,(10,5),"Número ASes Origen distintos",
                "Monitor",title,num_imagen,fecha,url_imagen,bar_width,opacity,'vertical')
...
[num_ASes,num_no_prepending,num_algunos_prepending,num_todos_prep,
 num_no_prepending_filt,num_algunos_prepending_filt,
 num_todos_prep_filt] = informacion_ASorigen_prepending_siempre_cuantos_monitores(data_origen)
