...
# -*- coding: utf-8 -*-
"""
Funcion que utiliza la informacion de prefijos mas especificos presente en ficheros CSV de cada colector independiente
Agrupando la informacion por AS que origina anuncios y almacenando con un formato mas sencillo de agrupar/procesar/representar 
posteriormente
...
'''almacenar_diccionario_mas_especificos_final:
	...
	A un formato:
		...
		- Values: 1. AS origen (Por arquitectura de DataFrames)
				  ...
				  8. Apariciones de prefijos bases distintos sobre los que se observa AS_PATH prepending
				  9. Apariciones de prefijos especificos distintos sobre los que se observa AS_PATH prepending
...
def almacenar_diccionario_mas_especificos_final(dicc_ASOrigen_esp):
    ...
    for i,v in dicc_ASOrigen_esp.items():
		...
		# Si el AS origen  anuncia mas de un prefijo, hay que comprobar que prefijos son base y cuales más especificos (si los hay)
			...
			#Ordeno la lista de mascaras para manejar primero las mascaras con longitud mas pequeña (y asi asegurarme
            ...
            for x in lista_mask:
				...
				#La primera mascara siempre es un prefijo base (al ser la mas pequeña) == La longitud de prefijo menor
                if x == lista_mask[0]:
				...
				#Si no es la primera mascara de la lista, compruebo si es un prefijo mas especifico
					...
					# Si coincide es un prefijo mas especifico y se añade a la lista de prefijos mas especificos
					# Si no coincide, es un prefijo base nuevo  y se añade a la lista de prefijos
            ...
            for z,k in v[1].items():
				...
				#Si la mascara pertenece a la lista de prefijos base se añade su contribucion a este grupo.
                if z in lista_bases:
				...
				#Si la mascara pertenece a la lista de prefijos mas especificos se añade su contribucion a este grupo.
                else:
...
dataframe_csv_mas_especificos_ipv4.to_csv("ResultadosCSV/Mas_Especificos/DiaRCC/MasEspecificos_ipv4_global_" + fecha  +  ".csv",index=False)
dataframe_csv_mas_especificos_ipv6.to_csv("ResultadosCSV/Mas_Especificos/DiaRCC/MasEspecificos_ipv6_global_" + fecha +  ".csv",index=False)
