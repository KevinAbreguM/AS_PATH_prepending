...
'''almacenar_diccionarios_longitudes_prefijo:
	...
	y otra de manera global.
		- Clave: El elemento de longitud de prefijo (OPCIONAL: Se añade tambien el monitor como parte de la clave)
				(OPCIONAL 2: Se añade tambien como clave la longitud de AS_PATH)
		- Values: 1. Longitud de prefijo + monitor <- (simplemente por arquitectura de diseño se repite la clave como value)
...
'''almacenar_dicc_ASPATHs:
	Funcion que almacena en un diccionario informacion relativa al atributo AS_PATH
		...
		- Values: 1. Atributo AS_PATH (por arquitectura de diseño se repite la clave como value)
...
def almacenar_dicc_ASPATHs(dicc_ASPATHs,data_aspath,prefijo_total,prepending,repeticiones,valor_mas_alto_prep,
	...
	# Si  se ha creado el diccionario con ese AS_PATH como clave, se actualizan los valores
		...
		#Compruebo si el prefijo ya habia sido añadido previamente
        if not prefijo[0] in lista_auxiliar_2:
        
            lista_auxiliar_2.append(prefijo[0])
			
        # Compruebo si el monitor ya ha sido añadido 
...
'''almacenar_diccionario_transito:
	...
	Es decir, ASes que no  originan rutas ni son el propio monitor.
		...
		- Values: 1. Monitor (por arquitectura de diseño se repite la clave como value)
				  2. AS transito (por arquitectura de diseño se repite la clave como value)
...
'''almacenar_dicc_monitor_prefijo_vecino
	Funcion que almacena en un diccionario informacion relativa a prefijos anunciados + ASes vecinos por monitor.
		...
		- Values: 1. Monitor (por arquitectura de diseño se repite la clave como value)
				  2. Prefijo anunciado (por arquitectura de diseño se repite la clave como value)
				  3. AS vecino (por arquitectura de diseño se repite la clave como value)
...
'''almacenar_dicc_pref_vecinos
	Funcion que almacena en un diccionario informacion relativa a prefijos anunciados + ASes vecinos.
		...
		- Values: 1. Prefijo anunciado (por arquitectura de diseño se repite la clave como value)
				  2. AS vecino (por arquitectura de diseño se repite la clave como value)
				  3. Veces que se realiza AS_PATH prepending
